...
/* =====================================================================
   PAGE: Event Attribute System (#/events)
   ===================================================================== */
register('#/events', renderEventsPage);
...
async function renderEventsPage() {
  ...
  const [categories, stats] = await Promise.all([
    API.get('/api/events/categories').catch(() => []),
    API.get('/api/events/stats').catch(() => null),
  ]);

  main.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
      <div>
        <h1 style="font-size:1.5rem;font-weight:700">事件属性体系</h1>
        <p style="color:var(--gray-500);font-size:.9rem;margin-top:4px">管理事件大类和小类，构建事件标注体系</p>
      </div>
    </div>
    ...
    <div class="grid grid-4" style="margin-bottom:24px">
      <div class="stat-box">
        <div class="stat-num" style="color:var(--primary)">${stats.total_categories}</div>
        <div class="stat-label">事件大类</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--success)">${stats.total_subcategories}</div>
        <div class="stat-label">事件小类</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--warning)">${stats.total_templates}</div>
        <div class="stat-label">句子模板</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--primary)">${stats.verified_templates}</div>
        <div class="stat-label">已审核</div>
      </div>
    </div>
    ` : ''}
    ...
    <div class="card">
      <div class="card-header">
        <span class="card-title">事件大类列表</span>
              ...
              ${categories.map((cat, i) => `
                <tr>
                  ...
                  <td style="font-weight:500">
                    <a href="#/events/category/${cat.id}" style="color:var(--primary);text-decoration:none">
                      ${escHtml(cat.name)}
                    </a>
                  </td>
                  <td><code style="font-size:.8rem;background:var(--gray-100);padding:2px 6px;border-radius:4px">${escHtml(cat.code)}</code></td>
                  <td><span class="badge badge-primary">${cat.subcategory_count}</span></td>
                  <td>
                  ...
                  <td>
                    <div style="display:flex;gap:4px">
                      <button class="btn btn-sm btn-secondary" onclick="showEditCategoryModal(${cat.id})">编辑</button>
                      <button class="btn btn-sm btn-danger-outline" onclick="deleteCategory(${cat.id})">删除</button>
                    </div>
                  </td>
                </tr>
              `).join('')}
...
window.showAddCategoryModal = function () {
  openModal(`
    <div class="modal-title">新增事件大类</div>
    <div class="form-group">
      <label>大类名称 *</label>
      <input type="text" id="cat-name" placeholder="例如：政治、经济、文化">
    </div>
    <div class="form-group">
      <label>编码 *</label>
      <input type="text" id="cat-code" placeholder="例如：politics, economy, culture">
    </div>
    <div class="form-group">
      <label>描述</label>
      <textarea id="cat-desc" placeholder="大类描述"></textarea>
    </div>
    <div class="form-group">
      <label>排序</label>
    ...
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doAddCategory()">创建</button>
    </div>
  `);
};
...
window.showEditCategoryModal = async function (id) {
  const category = await API.get(`/api/events/categories/${id}`).catch(() => null);
  if (!category) {
    toast('大类不存在', 'error');
    return;
  }
  openModal(`
    <div class="modal-title">编辑事件大类</div>
    <div class="form-group">
      <label>大类名称</label>
      <input type="text" id="cat-name" value="${escHtml(category.name)}">
    </div>
    <div class="form-group">
      <label>描述</label>
      <textarea id="cat-desc">${escHtml(category.description)}</textarea>
    </div>
    <div class="form-group">
      <label>排序</label>
      <input type="number" id="cat-sort" value="${category.sort_order}" min="0">
    </div>
    <div class="form-group">
      <label>状态</label>
      <select id="cat-active">
        <option value="true" ${category.is_active ? 'selected' : ''}>启用</option>
        <option value="false" ${!category.is_active ? 'selected' : ''}>停用</option>
      </select>
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doEditCategory(${id})">保存</button>
    </div>
  `);
};

window.doEditCategory = async function (id) {
  const name = document.getElementById('cat-name').value.trim();
  if (!name) {
    toast('请填写名称', 'error');
    return;
  ...
  try {
    await API.put(`/api/events/categories/${id}`, {
      name,
      description: document.getElementById('cat-desc').value.trim(),
      sort_order: parseInt(document.getElementById('cat-sort').value) || 0,
...
window.deleteCategory = async function (id) {
  if (!confirm('确定要删除这个事件大类吗？删除后相关小类也会被删除。')) return;
  try {
...
/* =====================================================================
   PAGE: Event Category Detail (#/events/category/:id)
   ===================================================================== */
register('#/events/category', renderEventCategoryPage);

async function renderEventCategoryPage(hash) {
  const categoryId = parseInt(hash.split('/')[3]);
  const main = document.getElementById('main-content');
  main.innerHTML = `<div class="spinner" style="margin:60px auto;display:block"></div>`;

  const [category, subcategories] = await Promise.all([
    API.get(`/api/events/categories/${categoryId}`).catch(() => null),
    API.get(`/api/events/categories/${categoryId}/subcategories`).catch(() => []),
  ]);

  if (!category) {
    main.innerHTML = '<div class="empty-state"><p>大类不存在</p></div>';
    return;
  }

  main.innerHTML = `
    <div class="breadcrumb">
      <a href="#/events">事件体系</a>
      <span>›</span>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
      <div>
        <h1 style="font-size:1.4rem;font-weight:700">${escHtml(category.name)}</h1>
        <p style="color:var(--gray-500);font-size:.9rem;margin-top:4px">
          编码: <code>${escHtml(category.code)}</code> | ${escHtml(category.description || '暂无描述')}
        </p>
      </div>
    ...
    <div class="card">
      <div class="card-header">
        <span class="card-title">事件小类列表</span>
        <span style="color:var(--gray-400);font-size:.85rem">${subcategories.length} 个小类</span>
      </div>
            ...
            <tbody>
              ${subcategories.map((sub, i) => `
                <tr>
                  <td>${i + 1}</td>
                  <td style="font-weight:500">
                    <a href="#/events/subcategory/${sub.id}" style="color:var(--primary);text-decoration:none">
                      ${escHtml(sub.name)}
                    </a>
                  </td>
                  <td><code style="font-size:.8rem;background:var(--gray-100);padding:2px 6px;border-radius:4px">${escHtml(sub.code)}</code></td>
                  <td><span class="badge badge-primary">${sub.template_count}</span></td>
                  <td>
                    <span class="badge ${sub.is_active ? 'badge-success' : 'badge-gray'}">
                      ${sub.is_active ? '启用' : '停用'}
                    </span>
                  </td>
                  ...
                  <td>
                    <div style="display:flex;gap:4px">
                      <button class="btn btn-sm btn-secondary" onclick="showEditSubcategoryModal(${sub.id})">编辑</button>
                      <button class="btn btn-sm btn-danger-outline" onclick="deleteSubcategory(${sub.id}, ${categoryId})">删除</button>
                    </div>
                  </td>
                </tr>
              `).join('')}
...
window.showAddSubcategoryModal = function (categoryId) {
  openModal(`
    <div class="modal-title">新增事件小类</div>
    <div class="form-group">
      <label>小类名称 *</label>
      <input type="text" id="sub-name" placeholder="例如：常委会议、政府常务会">
    </div>
    <div class="form-group">
      <label>编码 *</label>
      <input type="text" id="sub-code" placeholder="例如：standing_committee, government_meeting">
    </div>
    <div class="form-group">
      <label>描述</label>
      <textarea id="sub-desc" placeholder="小类描述"></textarea>
    </div>
    <div class="form-group">
      <label>排序</label>
      <input type="number" id="sub-sort" value="0" min="0">
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doAddSubcategory(${categoryId})">创建</button>
    </div>
  `);
};

window.doAddSubcategory = async function (categoryId) {
  ...
  try {
    await API.post('/api/events/subcategories', {
    ...
    closeModal();
    toast('小类创建成功', 'success');
    renderEventCategoryPage(`#/events/category/${categoryId}`);
  } catch (e) {
    toast(e.message, 'error');
  }
};

window.showEditSubcategoryModal = async function (id) {
  const subcategory = await API.get(`/api/events/subcategories/${id}`).catch(() => null);
  if (!subcategory) {
    toast('小类不存在', 'error');
    return;
  }
  openModal(`
    <div class="modal-title">编辑事件小类</div>
    <div class="form-group">
      <label>小类名称</label>
      <input type="text" id="sub-name" value="${escHtml(subcategory.name)}">
    </div>
    <div class="form-group">
      <label>描述</label>
      <textarea id="sub-desc">${escHtml(subcategory.description)}</textarea>
    </div>
    <div class="form-group">
      <label>排序</label>
      <input type="number" id="sub-sort" value="${subcategory.sort_order}" min="0">
    </div>
    <div class="form-group">
      <label>状态</label>
      <select id="sub-active">
        <option value="true" ${subcategory.is_active ? 'selected' : ''}>启用</option>
        <option value="false" ${!subcategory.is_active ? 'selected' : ''}>停用</option>
      </select>
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doEditSubcategory(${id}, ${subcategory.category_id})">保存</button>
    </div>
  `);
};

window.doEditSubcategory = async function (id, categoryId) {
  const name = document.getElementById('sub-name').value.trim();
  if (!name) {
    toast('请填写名称', 'error');
    return;
  ...
  try {
    await API.put(`/api/events/subcategories/${id}`, {
      name,
      description: document.getElementById('sub-desc').value.trim(),
      sort_order: parseInt(document.getElementById('sub-sort').value) || 0,
      is_active: document.getElementById('sub-active').value === 'true',
    });
    closeModal();
    toast('保存成功', 'success');
    renderEventCategoryPage(`#/events/category/${categoryId}`);
...
window.deleteSubcategory = async function (id, categoryId) {
  if (!confirm('确定要删除这个事件小类吗？删除后相关模板也会被删除。')) return;
  try {
    await API.delete(`/api/events/subcategories/${id}`);
    toast('小类已删除', 'success');
    renderEventCategoryPage(`#/events/category/${categoryId}`);
  } catch (e) {
    toast(e.message, 'error');
...
/* =====================================================================
   PAGE: Sentence Templates (#/templates)
   ===================================================================== */
register('#/templates', renderTemplatesPage);
...
async function renderTemplatesPage() {
  ...
  window._templateCategories = categories;

  main.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
      <div>
        <h1 style="font-size:1.5rem;font-weight:700">句子模板管理</h1>
        <p style="color:var(--gray-500);font-size:.9rem;margin-top:4px">管理和标注句子模板，用于事件类型标注</p>
      </div>
      <div style="display:flex;gap:8px">
        <button class="btn btn-secondary" onclick="showBatchImportModal()">
          📥 批量导入
        </button>
    ...
    <div class="grid grid-4" style="margin-bottom:24px">
      <div class="stat-box">
        <div class="stat-num">${stats.total_templates}</div>
        <div class="stat-label">总模板数</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--warning)">${stats.total_templates - stats.cleaned_templates}</div>
        <div class="stat-label">待清洗</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--primary)">${stats.cleaned_templates - stats.verified_templates}</div>
        <div class="stat-label">待审核</div>
      </div>
      <div class="stat-box">
        <div class="stat-num" style="color:var(--success)">${stats.verified_templates}</div>
        <div class="stat-label">已审核</div>
      </div>
    </div>
    ` : ''}
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <div class="form-group" style="min-width:200px;margin-bottom:0">
          <label>大类</label>
          <select id="filter-category" onchange="filterTemplates()">
        ...
        <div class="form-group" style="min-width:200px;margin-bottom:0">
          <label>状态</label>
          <select id="filter-status" onchange="filterTemplates()">
        ...
        <div class="form-group" style="min-width:200px;margin-bottom:0">
          <label>每页数量</label>
          <select id="filter-page-size" onchange="filterTemplates()">
        ...
        <div style="display:flex;align-items:flex-end">
          <button class="btn btn-primary" onclick="filterTemplates()">查询</button>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:20px">
      <div class="card-header">
        ...
        <span id="template-count" style="color:var(--gray-400);font-size:.85rem">0 条</span>
      </div>
...
window.filterTemplates = async function () {
  const container = document.getElementById('templates-table-container');
  const categoryId = document.getElementById('filter-category').value;
  const status = document.getElementById('filter-status').value;
  const pageSize = parseInt(document.getElementById('filter-page-size').value);
  ...
  if (categoryId) {
    subcategories = await API.get(`/api/events/categories/${categoryId}/subcategories`).catch(() => []);
    for (const sub of subcategories) {
      let isCleaned = null;
      let isVerified = null;
      if (status === 'cleaned') isCleaned = true;
      if (status === 'verified') isVerified = true;
      if (status === 'pending') { isCleaned = true; isVerified = false; }

      const subTemplates = await API.get(
        `/api/events/subcategories/${sub.id}/templates?page=1&page_size=${pageSize}` +
        (isCleaned !== null ? `&is_cleaned=${isCleaned}` : '') +
        (isVerified !== null ? `&is_verified=${isVerified}` : '')
      ).catch(() => []);
      templates = templates.concat(subTemplates.slice(0, pageSize));
    }
  }

  const countEl = document.getElementById('template-count');
  if (countEl) countEl.textContent = `${templates.length} 条`;
        ...
        <tbody>
          ${templates.map(t => `
            <tr>
              ...
              <td>
                ${t.is_verified ? '<span class="badge badge-success">已审核</span>' : 
                  t.is_cleaned ? '<span class="badge badge-primary">已清洗</span>' : 
                  '<span class="badge badge-gray">待处理</span>'}
              </td>
              <td style="color:var(--gray-500);font-size:.85rem">${escHtml(t.source || '-')}</td>
              <td>
                <div style="display:flex;gap:4px">
                  <button class="btn btn-sm btn-primary" onclick="showEditTemplateModal(${t.id})">编辑</button>
                  ${!t.is_verified ? `<button class="btn btn-sm btn-success" onclick="verifyTemplate(${t.id}, true)">审核</button>` : ''}
                </div>
              </td>
            </tr>
          `).join('')}
...
window.showAddTemplateModal = function () {
  const categories = window._templateCategories || [];
  openModal(`
    <div class="modal-title">新增句子模板</div>
    <div class="form-group">
      <label>所属大类 *</label>
      <select id="tmpl-category" onchange="updateSubcategoryOptions()">
    ...
    <div class="form-group">
      <label>所属小类 *</label>
      <select id="tmpl-subcategory">
        <option value="">请先选择大类</option>
      </select>
    </div>
    <div class="form-group">
      <label>句子内容 *</label>
      <textarea id="tmpl-content" rows="4" placeholder="请输入句子内容"></textarea>
    </div>
    <div class="form-group">
      <label>来源</label>
      <input type="text" id="tmpl-source" placeholder="例如：新疆日报">
    </div>
    <div class="form-group">
      <label>来源URL</label>
      <input type="text" id="tmpl-source-url" placeholder="可选">
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doAddTemplate()">创建</button>
    </div>
  `);
};

window.updateSubcategoryOptions = async function () {
  const categoryId = document.getElementById('tmpl-category').value;
  const subSelect = document.getElementById('tmpl-subcategory');
  ...
  const subcategories = await API.get(`/api/events/categories/${categoryId}/subcategories`).catch(() => []);
  subSelect.innerHTML = `
    <option value="">请选择小类</option>
    ${subcategories.map(s => `<option value="${s.id}">${escHtml(s.name)}</option>`).join('')}
  `;
};

window.doAddTemplate = async function () {
  const subcategoryId = document.getElementById('tmpl-subcategory').value;
  const content = document.getElementById('tmpl-content').value.trim();

  if (!subcategoryId || !content) {
    toast('请选择小类并填写句子内容', 'error');
    return;
  }
  ...
  try {
    await API.post('/api/events/templates', {
      ...
      content,
      source: document.getElementById('tmpl-source').value.trim() || null,
      source_url: document.getElementById('tmpl-source-url').value.trim() || null,
      is_cleaned: true,
    });
    closeModal();
...
window.showEditTemplateModal = async function (id) {
  const template = await API.get(`/api/events/templates/${id}`).catch(() => null);
  if (!template) {
    toast('模板不存在', 'error');
    return;
  }
  ...
  const categories = window._templateCategories || [];
  openModal(`
    <div class="modal-title">编辑句子模板</div>
    <div class="form-group">
      <label>句子内容</label>
      <textarea id="tmpl-edit-content" rows="4">${escHtml(template.content)}</textarea>
    </div>
    <div class="form-group">
      <label>状态</label>
      <div style="display:flex;gap:16px">
        <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
          <input type="checkbox" id="tmpl-edit-cleaned" ${template.is_cleaned ? 'checked' : ''}>
          已清洗
        </label>
        <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
          <input type="checkbox" id="tmpl-edit-verified" ${template.is_verified ? 'checked' : ''}>
          已审核
        </label>
      </div>
    </div>
    <div class="form-group">
      <label>来源</label>
      <input type="text" id="tmpl-edit-source" value="${escHtml(template.source || '')}">
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="doEditTemplate(${id})">保存</button>
    </div>
  `);
};

window.doEditTemplate = async function (id) {
  const content = document.getElementById('tmpl-edit-content').value.trim();
  if (!content) {
    toast('请填写句子内容', 'error');
    return;
  }
  ...
  try {
    await API.put(`/api/events/templates/${id}`, {
      content,
      is_cleaned: document.getElementById('tmpl-edit-cleaned').checked,
      is_verified: document.getElementById('tmpl-edit-verified').checked,
      source: document.getElementById('tmpl-edit-source').value.trim() || null,
    });
    closeModal();
    toast('保存成功', 'success');
    filterTemplates();
...
window.verifyTemplate = async function (id, verified) {
  if (!confirmed = confirm(verified ? '确定要审核通过这个模板吗？' : '确定要取消审核吗？')) return;
  try {
    await API.put(`/api/events/templates/${id}`, {
      is_verified: verified,
    });
    toast(verified ? '审核通过' : '已取消审核', 'success');
    filterTemplates();
  } catch (e) {
    toast(e.message, 'error');
  }
...
window.showBatchImportModal = function () {
  const categories = window._templateCategories || [];
  openModal(`
    <div class="modal-title">批量导入句子模板</div>
    <div class="form-group">
      <label>所属大类 *</label>
      <select id="batch-category" onchange="updateBatchSubcategoryOptions()">
        <option value="">请选择大类</option>
        ${categories.map(c => `<option value="${c.id}">${escHtml(c.name)}</option>`).join('')}
      </select>
    </div>
    <div class="form-group">
      <label>所属小类 *</label>
      <select id="batch-subcategory">
        <option value="">请先选择大类</option>
      </select>
    </div>
    <div class="form-group">
      <label>句子列表（每行一个句子）*</label>
      <textarea id="batch-sentences" rows="8" placeholder="请输入句子，每行一个&#10;例如：&#10;自治区党委召开常委会议&#10;政府常务会议研究经济工作"></textarea>
    </div>
    <div class="form-group">
      <label>来源</label>
      <input type="text" id="batch-source" placeholder="例如：新疆日报">
    </div>
    <div class="modal-footer">
      <button class="btn btn-secondary" onclick="closeModal()">取消</button>
...
window.updateBatchSubcategoryOptions = async function () {
...
window.doBatchImport = async function () {
  const subcategoryId = document.getElementById('batch-subcategory').value;
  const sentencesText = document.getElementById('batch-sentences').value.trim();

  if (!subcategoryId || !sentencesText) {
    toast('请选择小类并输入句子', 'error');
    return;
  }

  const sentences = sentencesText.split('\n').map(s => s.trim()).filter(s => s.length > 0);
  if (sentences.length === 0) {
    toast('请输入至少一个句子', 'error');
    return;
  }

  const source = document.getElementById('batch-source').value.trim() || null;
  ...
  try {
    await API.post('/api/events/templates/bulk', templates);
    closeModal();
    toast(`成功导入 ${sentences.length} 个句子模板`, 'success');
    filterTemplates();
  } catch (e) {
    toast(e.message, 'error');
  }
